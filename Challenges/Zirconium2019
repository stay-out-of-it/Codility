...
2.  Given A = [7, 1, 4, 4], B = [5, 3, 4, 3] and F = 2, the function should return 18. 
    The 0th and 3rd developers should be assigned to the front-end team and the 1st 
    and 2nd developers should be assigned to the back-end team.
...
def solution(A, B, F):
    sorted_list = sorted((B[i] - A[i], A[i], B[i]) for i in range(len(A)))
    first_F_max_sum_A = sum(map(lambda x: x[1], a[:F]))
    last_F_from_B = sum(map(lambda x: x[2], a[F:]))
    return first_F_max_sum_A + last_F_from_B
